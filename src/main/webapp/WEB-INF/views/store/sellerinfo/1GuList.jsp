		...
		<ul class="pagination">
			<c:choose>
				<c:when test="${startPageNum ne 1 }">
					<li><a 
						href="/ajax/selectGulist?pageNum=${startPageNum-1 }&searchType=${searchType }">
							&laquo; </a></li>
				</c:when>
				<c:otherwise>
			...
			<c:forEach var="i" begin="${startPageNum }" end="${endPageNum }"
				...
				<c:choose>
					<c:when test="${i eq pageNum }">
						<li class="active">
							<a onclick="clickGuPage('${i }','${open_close}' );">${i }</a>
							
						</li>
					</c:when>
					<c:otherwise>
						<li><a onclick="clickGuPage('${i }','${open_close}' );">${i }</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:choose>
				<c:when test="${endPageNum lt totalPageCount }">
					<li><a
						href="/ajax/selectGulist?pageNum=${endPageNum+1 }&open_close=${open_close}">
							&raquo; </a></li>
				</c:when>
				<c:otherwise>
