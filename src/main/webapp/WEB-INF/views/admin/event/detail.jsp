		...
		<div class="content_wrap content_event_detail">
			...
			<div class="container">
				...
				<c:if test="${eventDto.prevNum ne 0 }">
					<a
						href="detail.do?num=${eventDto.prevNum }&condition=${condition}&keyword=${encodedKeyword}">이전글</a>
				</c:if>

				<c:if test="${eventDto.nextNum ne 0 }">
					<a
						href="detail.do?num=${eventDto.nextNum }&condition=${condition}&keyword=${encodedKeyword}">다음글</a>
				</c:if>
...
function deleteConfirm(){
	...
	if(isDelete){
		location.href="${pageContext.request.contextPath }/cafe/delete.do?index=${eventDto.index}";
	}
}
</script>
